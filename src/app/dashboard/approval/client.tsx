...
const TRANSPORT_LABEL: Record<string, string> = {
  DARAT: "Darat", UDARA: "Udara", KERETA: "Kereta", LAUT: "Laut",
}
...
function getInitials(nama: string) {
  return nama.split(" ").map((n: string) => n[0]).slice(0, 2).join("").toUpperCase()
}
...
function InfoCell({
  label, value, valueNode, icon, noBorderRight, noBorderBottom,
}: {
  label: string
  value?: string
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        fontSize: "10.5px", color: "#8f95a3", fontWeight: 600,
...
export default function ApprovalClient({
  ...
  function openReview(item: PendingItem) {
    setSelected(item); setCatatan(""); setActionType(null); setError(null)
  }
  function closeModal() {
    setSelected(null); setCatatan(""); setActionType(null); setError(null)
  }

  function handleKeputusan(keputusan: "APPROVED" | "REJECTED") {
    if (!selected) return
    if (keputusan === "REJECTED" && !catatan.trim()) {
      setError("Catatan wajib diisi saat menolak pengajuan"); return
    }
    setError(null)
    startTransition(async () => {
      try {
        ...
        setRiwayat(prev => [{
          id: json.approvalId, keputusan,
          catatan: catatan.trim() || null,
          approvedAt: new Date().toISOString(),
          sppd: {
            nomorSppd: selected.nomorSppd, tujuan: selected.tujuan,
            anggaran: selected.anggaran, tglBerangkat: selected.tglBerangkat,
            tglKembali: selected.tglKembali, user: selected.user,
          },
        }, ...prev])
    ...
    <div>
      ...
      <div style={{
        ...
        {[
          ...
          <button key={t.key} onClick={() => setTab(t.key as "antrian" | "riwayat")}
            style={{
        ...
        <div style={{
          ...
          {pending.length === 0 ? (
            <div style={{
              ...
              <div style={{
                width: 48, height: 48, borderRadius: "12px", background: "#f0fdf4",
              ...
              <div style={{ fontSize: "13px", color: "#8f95a3" }}>Semua pengajuan sudah diproses</div>
            </div>
          ) : (
            pending.map((item, idx) => {
              ...
              return (
                ...
                >
                  <div style={{
                    width: 42, height: 42, borderRadius: "12px", background: "#eef1f8",
                    flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center",
                    fontSize: "13px", fontWeight: 700, color: "#00205b",
                  }}>
                    {getInitials(item.user.nama)}
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "3px" }}>
                      <span style={{ fontSize: "14px", fontWeight: 700, color: "#1a1f36" }}>{item.user.nama}</span>
                      {item.user.jabatan && (
                        <span style={{
                          fontSize: "11px", color: "#8f95a3", background: "#f3f4f6",
                          padding: "2px 7px", borderRadius: "20px", fontWeight: 500,
                        }}>{item.user.jabatan}</span>
                      )}
                    </div>
                    <div style={{
                      fontSize: "12px", color: "#6b7280", marginBottom: "5px",
                      ...
                      <span style={{ color: "#d1d5db" }}>·</span>
                      <span>{formatTanggal(item.tglBerangkat)} — {formatTanggal(item.tglKembali)}</span>
                      <span style={{
                        background: "#eff6ff", color: "#1d4ed8",
                        padding: "1px 6px", borderRadius: "20px", fontSize: "11px", fontWeight: 600,
                      }}>{hariItem} hari</span>
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                      <TransportBadge transport={item.transport} />
                      <span style={{ fontSize: "12px", color: "#8f95a3" }}>{formatRupiah(item.anggaran)}</span>
                    </div>
                  </div>
                  ...
                  <div style={{ display: "flex", gap: "6px", flexShrink: 0 }}>
                    ...
                    <button onClick={() => { setSelected(item); setActionType("APPROVED") }} title="Setujui"
                      style={{
                    ...
                    <button onClick={() => { setSelected(item); setActionType("REJECTED") }} title="Tolak"
                      style={{
                        width: 34, height: 34, borderRadius: "8px",
                    ...
                    >
                      ...
                      <td style={{ padding: "13px 16px" }}>
                        <div style={{ display: "flex", flexDirection: "column", gap: "3px" }}>
                          ...
                          {r.catatan && (
                            <span style={{
                              fontSize: "11px", color: "#8f95a3", fontStyle: "italic",
      ...
      {selected && (
        ...
        >
          <div style={{
            ...
            boxShadow: "0 24px 64px rgba(0,0,0,0.18)", maxHeight: "90vh", overflow: "auto",
          }}>
            {/* Header */}
            <div style={{
            ...
            <div style={{ padding: "20px 24px 24px" }}>
              ...
              {/* Info Grid — box-shadow sebagai border agar tidak terpotong overflow:hidden */}
              <div style={{
                ...
                <InfoCell
                  label="DURASI" noBorderBottom icon={IconClock}
                  valueNode={
                    <span style={{
                      display: "inline-flex", alignItems: "center", gap: "6px",
                      fontSize: "13px", fontWeight: 700,
                      color: hariSelected > 3 ? "#d97706" : "#1a1f36",
                    }}>
                      {hariSelected} hari
                      {hariSelected > 3 && (
                        <span style={{
                          fontSize: "10px", color: "#d97706", background: "#fffbeb",
                          border: "1px solid #fde68a", padding: "1px 5px",
                ...
                />
                <InfoCell
                  label="ESTIMASI ANGGARAN" noBorderRight noBorderBottom icon={IconMoney}
                  valueNode={
              ...
              <div style={{ marginBottom: "12px" }}>
                ...
                <div style={{
                  fontSize: "13px", color: "#1a1f36", lineHeight: 1.65,
                  background: "#f9fafb", padding: "11px 14px", borderRadius: "10px",
                  border: "1px solid #eef0f4",
                }}>{selected.maksud}</div>
              </div>
              ...
              {selected.catatan && (
                <div style={{ marginBottom: "16px" }}>
                  ...
                  <div style={{
                    fontSize: "13px", color: "#1a1f36", lineHeight: 1.65,
                    background: "#f9fafb", padding: "11px 14px", borderRadius: "10px",
                    border: "1px solid #eef0f4", fontStyle: "italic",
              ...
              <div style={{ marginBottom: "16px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "6px", marginBottom: "6px" }}>
                  ...
                  {actionType === "REJECTED" && (
                    <span style={{
                      ...
                      border: "1px solid #fecaca", padding: "1px 7px",
                      borderRadius: "4px", fontWeight: 600,
                    }}>Wajib diisi</span>
                  )}
                </div>
                <textarea
                  ...
                  style={{
                    ...
                    boxSizing: "border-box", color: "#1a1f36",
                    transition: "border-color 150ms ease",
                  }}
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
                <button
                  ...
                  style={{
                    ...
                    transition: "background 150ms ease", opacity: isPending ? 0.6 : 1,
                  }}
